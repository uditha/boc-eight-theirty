...
interface InterBankTradingChartProps {
  data: {
    record_date: string;
...
export default function InterBankTradingChart({ data, title }: InterBankTradingChartProps) {
  ...
  const processedData = useMemo(() => {
    ...
    return data.map(item => ({
      ...
      buying: Number(item.inter_bank_buying),
      selling: -Number(item.inter_bank_selling), // Make selling negative for downward bars
      actualSelling: Number(item.inter_bank_selling), // Keep actual value for display
      spread: Number(item.inter_bank_selling) - Number(item.inter_bank_buying)
  ...
  const stats = useMemo(() => {
    ...
    console.log("Filtered data sample:", filteredData.slice(0, 3)); // Debug log
    ...
    console.log("Valid buying data count:", validBuyingData.length); // Debug log
    console.log("Valid selling data count:", validSellingData.length); // Debug log

    if (validBuyingData.length > 0) {
      console.log("Sample buying values:", validBuyingData.slice(0, 3).map(d => d.buying)); // Debug log
    }
    ...
    const avgNetAmount = validSpreadData.length > 0
      ? validSpreadData.reduce((sum, item) => sum + (item.buying - Math.abs(item.selling)), 0) / validSpreadData.length
      : 0;
  ...
  const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: any[] }) => {
    if (active && payload && payload.length) {
      ...
      const netAmount = buyingValue - sellingValue; // Buy - Sell = Net Amount
        ...
        <div className="py-2 px-3 bg-card/95 dark:bg-card/95 backdrop-blur border border-border rounded shadow-lg text-sm text-foreground">
          ...
          <div className="grid gap-y-1">
            ...
            <p className="text-chart-3 flex justify-between">
              Net Amount: <span className="font-medium ml-2">{formatCurrency(netAmount)}</span>
            </p>
          </div>
        </div>
      );
  ...
  return (
    <div className="w-full p-4 bg-card rounded-lg border border-border shadow-sm">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h2 className="text-base font-semibold">{title || 'Interbank Trading'}</h2>
          {stats && (
            <div className="flex text-xs space-x-3 text-muted-foreground mt-1">
              ...
              <span>Avg Net Amount: <span className="text-chart-3 font-medium">
